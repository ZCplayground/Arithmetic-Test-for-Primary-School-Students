...
{
    int i;
    string versionChoice;//1的话 中文 2 英文
    ...
    int val, true_Numble = 0, wrong_Numble;     //正确答案以及正确题目数量 
	double accuracy;     
    for (i = 1; i <= n; i++)
    {
       
    //    val=Question();      //准备第几题这个也放入中英文切换里，但是函数返回值只有一个不方便。 
    //    if(versionChoice=="Chi")
    //    {            
	...
	//	}
        ...
        cout << "第" << i << "题:" << expression << endl;   //准备第几题这个也放入中英文切换里，但是函数返回值只有一个不方便。
    ...
    cout << "\n结果统计：您做对了" << true_Numble << "题，做错了" << wrong_Numble << "题，正确率:" << accuracy << endl;
    cout << "拜拜！~" << endl;
    return 0;
}
